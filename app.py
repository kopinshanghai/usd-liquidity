...
<div class="insight-box">
    ...
    <div>
        <div class="insight-row">
            {tag_1}
            <span class="insight-text"><b>【第一部分：美联储资产负债表】</b> 净流动性 <b>${net_liq_val:.1f} B</b>（周度数据 {net_liq_diff_b:+.2f} B），ON RRP 余额 <b>${rrp_val:.2f} B</b>，准备金 <b>${res_val:.1f} B</b></span>
        </div>
        <div class="insight-row">
            {tag_2}
            <span class="insight-text"><b>【第二部分：芝加哥联储金融条件】</b> 标准NFCI <b>{nfci_val:+.2f}</b> ｜ 修正ANFCI <b>{anfci_val:+.2f}</b>（周度数据 {nfci_diff:+.2f}）</span>
        </div>
        <div class="insight-row">
            {tag_3}
            <span class="insight-text"><b>【第三部分：在岸资金价格】</b> SOFR 利率 <b>{sofr_val:.2f}%</b>，IORB 利率 <b>{iorb_val:.2f}%</b>，融资利差 <b>{spread_val:+.1f} bps</b>（周度数据 {spread_diff:+.2f} bps）</span>
        </div>
        <div class="insight-row">
            {tag_4}
            <span class="insight-text"><b>【第四部分：离岸资金价格】</b> EUR基差 <b>{basis_eur:.2f} bps</b>（周度数据 {basis_eur_diff:+.2f} bps），JPY基差 <b>{basis_jpy:.2f} bps</b>，央行互换 <b>{swpt_str_disp}</b>，DXY <b>{dxy_val:.2f}</b>，VIX <b>{vix_val:.2f}</b></span>
        </div>
    </div>
</div>
''', unsafe_allow_html=True)
...
st.markdown(f"""
* **资产负债表维度**：净流动性报 **${net_liq_val:.1f} B**（周度数据 {net_liq_diff_b:+.2f} B），总量整体维持宽松。
* **金融条件维度**：标准 NFCI 报 **{nfci_val:+.2f}**，广义金融压强维持宽松区间。
* **在岸价格维度**：SOFR 与 IORB 利差报 **{spread_val:+.1f} bps**（周度数据 {spread_diff:+.2f} bps），回购市场资金摩擦极低。
* **离岸价格维度**：EUR 基差 **{basis_eur:.2f} bps**（周度数据 {basis_eur_diff:+.2f} bps）、JPY 基差 **{basis_jpy:.2f} bps**，离岸美元存在潜在结构性紧缩压强。
""")
...
v2.metric("美联储净流动性", f"${net_liq_val:.1f} B", f"{net_liq_diff_b:+.2f} B (周度数据)", help="📌 **是什么**：注入金融市场的真正‘有效活水’。\n\n🎯 **怎么看**：与标普500/美股估值极强正相关，是风险资产大锚。")
v3.metric("美联储总资产", f"${walcl_val:.1f} B", f"{walcl_diff_b:+.2f} B (周度数据)", help="📌 **是什么**：美联储扩表/缩表的总阀门。\n\n🎯 **怎么看**：反映央行货币政策大周期（QE扩表放水 vs QT缩表抽水）。")
v4.metric("商业银行准备金", f"${res_val:.1f} B", f"{res_diff_b:+.2f} B (周度数据)", help="📌 **是什么**：商业银行做市与信贷扩张的储备金。\n\n🎯 **怎么看**：准备金低于临界线（如<3万亿）易引发在岸隔夜资金打架。")
v5.metric("ON RRP 逆回购", f"${rrp_val:.2f} B", f"{rrp_diff_b:+.2f} B (周度数据)", delta_color="inverse", help="📌 **是什么**：货币基金闲置资金的‘停泊池’。\n\n🎯 **怎么看**：RRP下降释放资金可抵消缩表；若耗尽将直接消耗准备金。")
v6.metric("TGA 财政部账户", f"${tga_val:.1f} B", f"{tga_diff_b:+.2f} B (周度数据)", delta_color="inverse", help="📌 **是什么**：美国财政部在美联储的活期账户。\n\n🎯 **怎么看**：财政发债/税收（TGA增加）抽水，财政发钱支出（TGA下降）放水。")
...
f2.metric("标准 NFCI 最新值", f"{nfci_val:+.2f}", f"{nfci_diff:+.2f} (周度数据)", delta_color="inverse", help="📌 **是什么**：美联储官方评估广义金融条件的综合指数 (NFCI)。\n\n🎯 **怎么看**：数值 < 0 代表宽松；数值 > 0 代表紧缩。")
f3.metric("修正 ANFCI 最新值", f"{anfci_val:+.2f}", f"{anfci_diff:+.2f} (周度数据)", delta_color="inverse", help="📌 **是什么**：芝加哥联储剔除宏观经济与通胀干扰后的‘修正金融条件指数’ (ANFCI)。\n\n🎯 **怎么看**：纯粹剥离经济基本面影响，评估金融体系自身的独立压强。")
...
p2.metric("SOFR - IORB 利差", f"{spread_val:+.1f} bps", f"{spread_diff:+.2f} bps (周度数据)", delta_color="inverse", help="📌 **是什么**：担保借贷成本与准备金收益的差值。\n\n🎯 **怎么看**：利差升至正值（>0）表明机构不惜溢价借钱，市场极度缺资金。")
p3.metric("SOFR 隔夜担保融资利率", f"{sofr_val:.2f}%", f"{sofr_diff_bps:+.2f} bps (周度数据)", help="📌 **是什么**：全美最核心的短端国债质押融资基准利率。\n\n🎯 **怎么看**：测量回购市场实际资金水温，替代了历史上的 LIBOR。")
p4.metric("EFFR 联邦基金有效利率", f"{effr_val:.2f}%", f"{effr_diff_bps:+.2f} bps (周度数据)", help="📌 **是什么**：美联储政策利率的目标核心中枢。\n\n🎯 **怎么看**：观察其与市场回购利率的偏离与走廊边界。")
...
o2.metric("EUR/USD 3M 基差", f"{basis_eur:.2f} bps", f"{basis_eur_diff:+.2f} bps (周度数据)", delta_color="normal" if basis_eur > -30 else "inverse", help="📌 **是什么**：欧洲机构获取美元的掉期额外溢价。\n\n🎯 **怎么看**：基差深度走负（<-30bps）意味着离岸发生美元挤兑。")
o3.metric("USD/JPY 3M 基差", f"{basis_jpy:.2f} bps", f"{basis_jpy_diff:+.2f} bps (周度数据)", delta_color="normal" if basis_jpy > -30 else "inverse", help="📌 **是什么**：日本机构/套息资金获取美元的掉期溢价。\n\n🎯 **怎么看**：捕捉亚洲区套息交易逆转与日系机构海外对冲压强。")
o4.metric("央行互换余额 (SWPT)", f"${swpt_val:.0f} M", f"{swpt_diff:+.0f} M (周度数据)", delta_color="inverse", help="📌 **是什么**：美联储向外国央行提供的终极救市通道。\n\n🎯 **怎么看**：常态保持为0；若突然飙升表明发生离岸流动性危机。")
o5.metric("发达国家美元指数", f"{dxy_val:.2f}", f"{dxy_diff:+.2f} (周度数据)", delta_color="inverse", help="📌 **是什么**：美联储官方发达经济体贸易加权美元指数 (DTWEXAFEGS)。\n\n🎯 **怎么看**：衡量美元在全球成熟市场资本流动中的真实购买力。")
o6.metric("VIX 恐慌指数", f"{vix_val:.2f}", f"{vix_diff:+.2f} (周度数据)", delta_color="inverse", help="📌 **是什么**：美股期权隐含波动率，避险情绪风向标。\n\n🎯 **怎么看**：VIX飙升(>20)引发海外抛资产囤美元，是离岸紧缩先导雷达。")
